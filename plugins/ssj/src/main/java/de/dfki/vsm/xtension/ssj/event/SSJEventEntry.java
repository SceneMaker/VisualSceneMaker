...
import de.dfki.vsm.xtension.ssj.event.data.SSJStringData;
 ...
 */
public final class SSJEventEntry extends SSJEventObject implements Comparable<SSJEventEntry>
{
    ...
    // The event data
    private SSJEventData mData;

    public SSJEventEntry()
    {
        // Default empty constructor
    }

    public SSJEventEntry(String mSender, String mEvent, String mFrom, String mDur, String mProb, String mType, String mState, String mGlue)
    {
    ...
    // Get the event data
    public final SSJEventData getData()
    {
        return mData;
    }

    public void setData(SSJEventData mData)
    {
        this.mData = mData;
    }

    @Override
    public int compareTo(final SSJEventEntry other)
    {
        ...
        else
        {
            ...
            {
                if (other.mState.equalsIgnoreCase("completed") && mState.equalsIgnoreCase("continued"))
                {
                    return -1;
                }
                else
    ...
    {
        writer.print("<event " + "sender=\"" + mSender + "\" " + "event=\"" + mEvent + "\" " + "from=\"" + mFrom + "\" " + "dur=\"" + mDur + "\" " + "prob=\"" + mProb + "\" " + "type=\"" + mType + "\" " + "state=\"" + mState + "\" " + "glue=\"" + mGlue + "\">");
        // Write the data
        if (mData != null)
        {
    ...
    public final void parseXML(final Element element) throws XMLParseError
    {
        ...
        if (element.getTagName().equals("event"))
        {
            ...
            mProb = String.format(Locale.US, "%.6f", Double.valueOf(element.getAttribute("prob").toLowerCase()));
            ...
            {
                mData = new SSJStringData(element.getTextContent().trim());
            }
            else if (mType.equalsIgnoreCase("ntuple") || mType.equalsIgnoreCase("map"))
            {
                final SSJTupleData data = new SSJTupleData();
