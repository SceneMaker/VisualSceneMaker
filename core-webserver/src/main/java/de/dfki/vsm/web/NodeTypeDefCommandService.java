...
/**
 * Handles SceneFlow.Node.TypeDef.* commands.
 */
public final class NodeTypeDefCommandService {
    ...
    public interface Context {
        ...
        DataTypeDefinition parseTypeDef(JSONObject source, StringBuilder error);
    ...
    public JSONObject dispatch(final String method,
        ...
        switch (method) {
            ...
            default:
                return context.errorResponse("BAD_REQUEST", "Unsupported type-def command: " + method);
        }
    }
    ...
    private JSONObject handleAdd(final JSONObject params,
        ...
        final JSONObject typeDefJson = params.optJSONObject("typeDef");
        ...
        if (typeDefJson == null) {
            return context.errorResponse("BAD_REQUEST", "Missing typeDef");
        }
        ...
        final DataTypeDefinition typeDef = context.parseTypeDef(typeDefJson, error);
        if (typeDef == null) {
            return context.errorResponse("TYPEDEF_INVALID", error.length() > 0 ? error.toString() : "Invalid type definition");
        }

        final List<DataTypeDefinition> list = dataNode.getTypeDefList();
        ...
        return snapshotAndRecord(context, project, pid, superNodeId, sceneFlow,
                "SceneFlow.Node.TypeDef.Add", params, broadcaster);
    }
    ...
    private JSONObject handleUpdate(final JSONObject params,
        ...
        if (typeDefJson == null || index < 0) {
            return context.errorResponse("BAD_REQUEST", "Missing typeDef or index");
        }
        ...
        final List<DataTypeDefinition> list = dataNode.getTypeDefList();
        if (index >= list.size()) {
            return context.errorResponse("TYPEDEF_NOT_FOUND", "Type definition not found at index: " + index);
        }
        ...
        return snapshotAndRecord(context, project, pid, superNodeId, sceneFlow,
                "SceneFlow.Node.TypeDef.Update", params, broadcaster);
    }
    ...
    private JSONObject handleDelete(final JSONObject params,
        ...
        return snapshotAndRecord(context, project, pid, superNodeId, sceneFlow,
                "SceneFlow.Node.TypeDef.Delete", params, broadcaster);
    }
    ...
    private JSONObject handleMove(final JSONObject params,
        ...
        if (from >= list.size() || to >= list.size()) {
            return context.errorResponse("TYPEDEF_NOT_FOUND", "Invalid index");
        }

        if (from != to) {
            final DataTypeDefinition entry = list.remove(from);
            list.add(to, entry);
        }

        return snapshotAndRecord(context, project, pid, superNodeId, sceneFlow,
                "SceneFlow.Node.TypeDef.Move", params, broadcaster);
    }
