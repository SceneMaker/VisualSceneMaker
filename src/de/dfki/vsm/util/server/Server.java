...
public class Server {
    ...
    protected synchronized void addConnection(Socket s, Service service) {
        ...
        if (mConnections.size() >= mMaxConnections) {
            try {
                ...
                mLogger.message("Connection refused to " + s.getInetAddress().getHostAddress() + ":" + s.getPort()
            ...
            mLogger.message("Connected to " + s.getInetAddress().getHostAddress() + ":" + s.getPort() + " on port "
    ...
    protected synchronized void endConnection(Connection c) {
        ...
        mLogger.message("Connection to " + c.getClientSocket().getInetAddress().getHostAddress() + ":"
                        + c.getClientSocket().getPort() + " closed.");
    }
    ...
    public synchronized void displayStatus(PrintWriter out) {
        ...
        while (conns.hasNext()) {
            ...
            out.print("CONNECTED TO " + c.getClientSocket().getInetAddress().getHostAddress() + ":"
                      + c.getClientSocket().getPort() + " ON PORT " + c.getClientSocket().getLocalPort()
                      + " FOR SERVICE " + c.getService().getClass().getName() + "\n");
        }
    }
    ...
    public synchronized void info() {
        ...
        for (Entry<Integer, Listener> entry : mServices.entrySet()) {
            infoString += "Service " + entry.getValue().getService().getClass().getName() + " is listening on port "
                          + entry.getKey() + "\r\n";
        }
        ...
        for (Connection conn : mConnections) {
            infoString += "Connected to " + conn.getClientSocket().getInetAddress().getHostAddress() + ":"
                          + conn.getClientSocket().getPort() + " on port " + conn.getClientSocket().getLocalPort()
                          + " for service " + conn.getService().getClass().getName() + "\r\n";
        }
