...
import de.dfki.vsm.runtime.error.InterruptException;
...
public class Process extends java.lang.Thread {
    ...
    private final Vector<Process> mChildThreadList
            = new Vector<Process>();
    private final Vector<Process> mAddChildThreadList
            = new Vector<Process>();
    ...
    public void checkStatus() throws TerminatedException, InterruptException {
        checkPaused();
        if (mIsTerminationRequested) {
            throw new TerminatedException(this, null);
        }
    ...
    public void handleStart() throws RunTimeException {
        /**
    ...
    public void handleInterruption() throws RunTimeException {
        /**
         * Set the new incoming edge to the interruptive edge that caused this interruption
         */
    ...
    private void execute() throws RunTimeException {
        mInterpreter.lock();
        try {
            checkStatus();
    ...
    /**
     *      */
    private void executeStartNodeList() throws InterruptException, TerminatedException, RunTimeException {
    ...
    private Vector<Node> computeStartNodeList() {
         ...
         */
        Collection<Node> commonStartNodeList
                = ((SuperNode) mCurrentNode).getStartNodeMap().values();
        /**
...
//    }
    ...
    /**
     *      */
    private void processVarDefList() throws RunTimeException {
    ...
    /**
     *      */
    private void processTimeoutConditionList() throws RunTimeException {
        for (VarDef varDef : mCurrentNode.getVarDefList()) {
    ...
    /**
     *      */
    private CEdge checkCEdgeList() throws RunTimeException {
        for (CEdge cedge : mCurrentNode.getCEdgeList()) {
            try {
                ...
                java.lang.String errorMsg = "An error occured while executing thread " + Process.currentThread().toString() + " : "
                        + "The condition '" + cedge.getCondition().getConcreteSyntax() + "' of the conditional edge from node '"
                        + cedge.getSource() + "' to node '" + cedge.getTarget() + "' could not be evaluated to a boolean value.";
