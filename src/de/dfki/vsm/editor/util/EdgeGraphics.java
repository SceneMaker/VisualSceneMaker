...
public class EdgeGraphics {
    ...
    public EdgeGraphics(Edge e, Point sourceDockpoint, Point targetDockpoint) {
        ...
        if (mDataEdge.getGraphics() != null) {
            Vector<de.dfki.vsm.model.sceneflow.graphics.edge.Point> curvePoints = mDataEdge.getGraphics().getArrow().getPointList();
    ...
    void computeBounds() {
        ...
        if (mEdge.getGraphics() != null) {
            ...
            bounds.add(this.mLeftCurve.x2 - visualBounds.width / 2 - 5, this.mLeftCurve.y2 - visualBounds.height / 2 - 2);
            bounds.add(this.mLeftCurve.x2 + visualBounds.width / 2 + 5, this.mLeftCurve.y2 + visualBounds.height / 2 + 2);
        }
    ...
    public void updateDrawingParameters() {
        if (!mPointingToSameNode) {
            mAbsoluteStartPos = (!mEdge.mCSPSelected) ? mSourceNode.getEdgeDockPoint(mEdge) : mAbsoluteStartPos;
            mAbsoluteEndPos = (!mEdge.mCEPSelected) ? mTargetNode.getEdgeDockPoint(mEdge) : mAbsoluteEndPos;
        } else {
            mAbsoluteStartPos = (!mEdge.mCSPSelected) ? mSourceNode.getEdgeDockPoint(mEdge) : mAbsoluteStartPos;
            mAbsoluteEndPos = (!mEdge.mCEPSelected) ? mTargetNode.getSelfPointingEdgeDockPoint(mEdge) : mAbsoluteEndPos;
        }
        computeCurve();
        updateDataModel();
        computeBounds();
    ...
    public void initCurve() {
        ...
        double scalingFactor = (mPointingToSameNode) ? 3 : ((distance / sNODEHEIGHT) - 0.5d);
        scalingFactor = (scalingFactor < 1.0d) ? 1.25d : scalingFactor;
    ...
    private void computeCurve() {
        ...
        // setup curve
        mCurve = new CubicCurve2D.Double(mCurveControlPoints[0].x, mCurveControlPoints[0].y, mCurveControlPoints[1].x, mCurveControlPoints[1].y,
                mCurveControlPoints[2].x, mCurveControlPoints[2].y, mCurveControlPoints[3].x, mCurveControlPoints[3].y);
    ...
    // 1. case - start node and target node are different
        ...
        if (!mSourceNode.equals(mTargetNode)) {
            ...
            for (Point p : freeSourceNodeDockPoints) {
                ...
                for (Point q : freeTargetNodeDockPoints) {
                    ...
                    actualDist += Point.distance(mSourceNode.getCenterPoint().x, mSourceNode.getCenterPoint().y, p.x, p.y);
                    actualDist += Point.distance(mTargetNode.getCenterPoint().x, mTargetNode.getCenterPoint().y, q.x, q.y);
                    dist = (dist == -1.0d) ? actualDist : dist;
                    if (actualDist < dist) {
                        dist = actualDist;
      ...
      // let the start and end point bet placed at least one third of the mean
            ...
            double minDist = (sNODEHEIGHT + sNODEWIDTH) / 2 / 3;
            for (Point p : freeSourceNodeDockPoints) {
    ...
    public boolean curveStartPointContainsPoint(Point point, int threshold) {
        ...
        if ((((int) mAbsoluteStartPos.x - threshold) < point.x) && (((int) mAbsoluteStartPos.x + threshold) > point.x)
                && (((int) mAbsoluteStartPos.y - threshold) < point.y) && (((int) mAbsoluteStartPos.y + threshold) > point.y)) {
            //Debug //System.out.println("\tyes!");
            return true;
        }
    ...
    public boolean curveEndPointContainsPoint(Point point, int threshold) {
        ...
        if ((((int) mAbsoluteEndPos.x - threshold) < point.x) && (((int) mAbsoluteEndPos.x + threshold) > point.x)
                && (((int) mAbsoluteEndPos.y - threshold) < point.y) && (((int) mAbsoluteEndPos.y + threshold) > point.y)) {
            //Debug //System.out.println("\tyes!");
            return true;
        }
        return false;
...
//     graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        ...
        for (t = k; t <= 1; t += k) {
            x2 = (mCoordList[0].x + t * (-mCoordList[0].x * 3 + t * (3 * mCoordList[0].x
                    - mCoordList[0].x * t))) + t * (3 * mCoordList[1].x + t * (-6 * mCoordList[1].x
                    + mCoordList[1].x * 3 * t)) + t * t * (mCoordList[2].x * 3 - mCoordList[2].x * 3 * t)
                    + mCoordList[3].x * t * t * t;
            y2 = (mCoordList[0].y + t * (-mCoordList[0].y * 3 + t * (3 * mCoordList[0].y
                    - mCoordList[0].y * t))) + t * (3 * mCoordList[1].y + t * (-6 * mCoordList[1].y
                    + mCoordList[1].y * 3 * t)) + t * t * (mCoordList[2].y * 3 - mCoordList[2].y * 3 * t)
                    + mCoordList[3].y * t * t * t;
...
//      graphics.drawPolygon(lineHull);
    ...
    public void updateDataModel() {
        ...
        de.dfki.vsm.model.sceneflow.graphics.edge.Graphics g = new de.dfki.vsm.model.sceneflow.graphics.edge.Graphics();
        de.dfki.vsm.model.sceneflow.graphics.edge.Arrow arrow = new de.dfki.vsm.model.sceneflow.graphics.edge.Arrow();
        Vector<de.dfki.vsm.model.sceneflow.graphics.edge.Point> xmlEdgePoints = new Vector<de.dfki.vsm.model.sceneflow.graphics.edge.Point>();

        de.dfki.vsm.model.sceneflow.graphics.edge.Point startPoint = new de.dfki.vsm.model.sceneflow.graphics.edge.Point();
        ...
        de.dfki.vsm.model.sceneflow.graphics.edge.Point endPoint = new de.dfki.vsm.model.sceneflow.graphics.edge.Point();
