...
public class CreateEdgeAction extends EdgeAction {
    ...
    public CreateEdgeAction(WorkSpace workSpace, Node sourceNode, Node targetNode, de.dfki.vsm.model.sceneflow.Edge dataEdge, Edge.TYPE type) {
        mWorkSpace = workSpace;
        ...
        if (mShowDialog) {
            switch (mGUIEdgeType) {
                ...
                case FEDGE:
                    ...
                    break;
                case TEDGE:
                    CreateTEdgeDialog tedgeDialog = new CreateTEdgeDialog(
                            mSourceGUINode.getDataNode(),
                            mTargetGUINode.getDataNode());
                    mDataEdge = tedgeDialog.run();
                    break;
                case CEDGE:
                    CreateCEdgeDialog cedgeDialog = new CreateCEdgeDialog(
                            mSourceGUINode.getDataNode(),
                            mTargetGUINode.getDataNode());
                    mDataEdge = cedgeDialog.run();
                    break;
                case PEDGE:
                    CreatePEdgeDialog pedgeDialog = new CreatePEdgeDialog(
                            mSourceGUINode.getDataNode(),
                            mTargetGUINode.getDataNode());
