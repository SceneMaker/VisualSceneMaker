...
public class PasteNodesAction extends EditorAction {
    ...
    protected void pasteNodes() {
        mSceneFlowEditor.setMessageLabelText((mWorkSpace.getClipBoard().size() > 1) ? "Pasting selected nodes" : "Pasting selected node");
        ...
        for (Node node : nodes) {
            ...
            if (mNodesCEdges.containsKey(node)) {
                ...
                for (CEdge c : ces) {
                    CreateEdgeAction cea = new CreateEdgeAction(mWorkSpace, mWorkSpace.getNode(node.getId()), mWorkSpace.getNode(c.getTarget()), c, de.dfki.vsm.editor.Edge.TYPE.CEDGE);
                    cea.run();
                }
            }
            // pedge
            if (mNodesPEdges.containsKey(node)) {
                ...
                for (PEdge p : pes) {
                    CreateEdgeAction cea = new CreateEdgeAction(mWorkSpace, mWorkSpace.getNode(node.getId()), mWorkSpace.getNode(p.getTarget()), p, de.dfki.vsm.editor.Edge.TYPE.PEDGE);
                    cea.run();
                }
            }
            // fedge
            if (mNodesFEdges.containsKey(node)) {
                ...
                for (FEdge f : fes) {
                    CreateEdgeAction cea = new CreateEdgeAction(mWorkSpace, mWorkSpace.getNode(node.getId()), mWorkSpace.getNode(f.getTarget()), f, de.dfki.vsm.editor.Edge.TYPE.FEDGE);
                    cea.run();
                }
            }
            // iedge
            if (mNodesIEdges.containsKey(node)) {
                ...
                for (IEdge i : ies) {
                    CreateEdgeAction cea = new CreateEdgeAction(mWorkSpace, mWorkSpace.getNode(node.getId()), mWorkSpace.getNode(i.getTarget()), i, de.dfki.vsm.editor.Edge.TYPE.IEDGE);
                    cea.run();
                }
            }
            // dedge
            if (mNodesDefaultEdge.containsKey(node)) {
                ...
                if (TEdge.class.isInstance(e)) {
                    cea = new CreateEdgeAction(mWorkSpace, mWorkSpace.getNode(node.getId()), mWorkSpace.getNode(e.getTarget()), e, de.dfki.vsm.editor.Edge.TYPE.TEDGE);
                } else {
                    cea = new CreateEdgeAction(mWorkSpace, mWorkSpace.getNode(node.getId()), mWorkSpace.getNode(e.getTarget()), e, de.dfki.vsm.editor.Edge.TYPE.EEDGE);
                }
                cea.run();
            }
        }
