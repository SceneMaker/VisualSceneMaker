...
//             mParentDataNode.removeNode(mDataNode);
        ...
        HashMap<String, de.dfki.vsm.model.sceneflow.diagram.nodes.BasicNode> startNodeMap = mParentDataNode.getStartNodeMap();
            ...
            if (startNodeMap.isEmpty()) {
                if (mParentDataNode.getNodeList().isEmpty()) {
                    if (mParentDataNode.getSuperNodeList().isEmpty()) {
                        ...
                        de.dfki.vsm.model.sceneflow.diagram.nodes.BasicNode newStartNode = mParentDataNode.getSuperNodeAt(0);

                        mParentDataNode.getStartNodeMap().put(newStartNode.getId(), newStartNode);
                    }
                } else {
                    for (de.dfki.vsm.model.sceneflow.diagram.nodes.BasicNode n : mParentDataNode.getNodeList()) {
                        if (n.isHistoryNode()) {
                            continue;
                        } else {
