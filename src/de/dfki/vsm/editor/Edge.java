...
public class Edge extends JComponent implements EventListener, Observer, MouseListener {
    ...
    public Edge(WorkSpace ws, de.dfki.vsm.model.sceneflow.Edge edge, TYPE type, Node sourceNode, Node targetNode, Point sourceDockPoint, Point targetDockpoint) {
        mDataEdge = edge;
        mWorkSpace = ws;
        mSourceNode = sourceNode;
        mTargetNode = targetNode;
        mType = type;
        mPointingToSameNode = (mTargetNode == mSourceNode) ? true : false;

        // Timer
        mVisualisationTimer = new Timer("Edge(" + mDataEdge.getSource() + "->" + mDataEdge.getTarget() + ")-Visualization-Timer");
...
//    revalidate();
    ...
    private void paintRoundedTextBadge(Graphics2D graphics, Point position, String text) {
        ...
        graphics.fillRect(position.x - mFontWidthCorrection - 2, position.y - halfTextHeight - 2, mFontWidthCorrection * 2 + 4, halfTextHeight * 2 + 4);
        graphics.setColor(mColor);
        graphics.setStroke(new BasicStroke(0.5f));
        graphics.drawRoundRect(position.x - mFontWidthCorrection - 2, position.y - halfTextHeight - 2, mFontWidthCorrection * 2 + 4, halfTextHeight * 2 + 4, 5, 5);
...
//        graphics.drawRect(bounds.x, bounds.y, bounds.width - 1, bounds.height - 1);
        graphics.setColor(mColor);
        graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        graphics.setStroke(new BasicStroke(sNODEWIDTH / 30.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));
        ...
        if (mIsSelected) {
            ...
            graphics.setStroke(new BasicStroke(0.5f));
            graphics.drawLine((int) mEg.mCurve.x1, (int) mEg.mCurve.y1, (int) mEg.mCurve.ctrlx1, (int) mEg.mCurve.ctrly1);
            graphics.drawLine((int) mEg.mCurve.x2, (int) mEg.mCurve.y2, (int) mEg.mCurve.ctrlx2, (int) mEg.mCurve.ctrly2);
            graphics.setStroke(new BasicStroke(sNODEWIDTH / 30.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));
            if (mCP1Selected) {
                graphics.setColor(mColor);
            }
        ...
        if (mVisualisationTask != null) {
            if (mVisualisationTask.getActivityTime() <= 20) {
                ...
                graphics.setColor(new Color(246 - gray, gray, gray, ((sACTIVITYTRACE) ? 100 : 5 * mVisualisationTask.getActivityTime())));
            } else {
                // visualise activity
