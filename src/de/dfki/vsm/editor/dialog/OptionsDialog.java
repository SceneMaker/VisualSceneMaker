	...
	private void initGeneralPanel() {
		...
		mXSDFileButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				JFileChooser file = new JFileChooser(Preferences.sUSER_DIR);
...
//    }
	private void savePreferences(boolean dispose) {
		//mLogger.message("\r\nSaving Preferences:");
		Preferences.setProperty("xmlns", mXMLNSTextField.getText().trim());
		Preferences.setProperty("xmlns_xsi", mXMLInstanceTextField.getText().trim());
		Preferences.setProperty("xsi_schemeLocation", mXSDFileTextField.getText().trim());
		Preferences.setProperty("node_width",
		  Integer.toString(((SpinnerNumberModel) mNodeSizeSpinner.getModel()).getNumber().intValue()));
		Preferences.setProperty("node_height",
		  Integer.toString(((SpinnerNumberModel) mNodeSizeSpinner.getModel()).getNumber().intValue()));
		Preferences.setProperty("grid_x",
		  Integer.toString(((SpinnerNumberModel) mGridScaleSpinner.getModel()).getNumber().intValue()));
		Preferences.setProperty("grid_y",
		  Integer.toString(((SpinnerNumberModel) mGridScaleSpinner.getModel()).getNumber().intValue()));
		Preferences.setProperty("workspace_fontsize",
		  Integer.toString(((SpinnerNumberModel) mWorkspaceFontSizeSpinner.getModel()).getNumber().intValue()));
		Preferences.setProperty("grid",
		  Boolean.toString(mGridCheckBox.isSelected()));
		Preferences.setProperty("visualization",
		  Boolean.toString(mVisualizationCheckBox.isSelected()));
		Preferences.setProperty("visualizationtrace",
		  Boolean.toString(mVisualizationTraceCheckBox.isSelected()));
		Preferences.setProperty("shownodeid",
		  Boolean.toString(mShowNodeIDCheckBox.isSelected()));
		Preferences.setProperty("showvariables",
		  Boolean.toString(mShowVariablesCheckBox.isSelected()));
		Preferences.setProperty("showsmartpathcalculations",
...
//        ((DefaultComboBoxModel) mScenePlayerComboBox.getModel()).removeAllElements();
		...
		for (Object keyObj : Preferences.getKeySet()) {
			...
			if (key.startsWith("recentfile")) {
				((DefaultListModel) mRecentFileList.getModel()).addElement(Preferences.getProperty(key));
			...
			//            }
				...
				((SpinnerNumberModel) mNodeSizeSpinner.getModel()).setValue(Integer.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("node_height")) {
				((SpinnerNumberModel) mNodeSizeSpinner.getModel()).setValue(Integer.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("grid_x")) {
				((SpinnerNumberModel) mGridScaleSpinner.getModel()).setValue(Integer.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("grid_y")) {
				((SpinnerNumberModel) mGridScaleSpinner.getModel()).setValue(Integer.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("grid")) {
				mGridCheckBox.setSelected(Boolean.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("visualization")) {
				mVisualizationCheckBox.setSelected(Boolean.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("visualizationtrace")) {
				mVisualizationTraceCheckBox.setSelected(Boolean.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("shownodeid")) {
				mShowNodeIDCheckBox.setSelected(Boolean.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("showvariables")) {
				mShowVariablesCheckBox.setSelected(Boolean.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("showsmartpathcalculations")) {
				mShowSmartPathDebugCheckBox.setSelected(Boolean.valueOf(Preferences.getProperty(key)));
			} else if (key.equals("xmlns")) {
				mXMLNSTextField.setText(Preferences.getProperty(key));
			} else if (key.equals("xmlns_xsi")) {
				mXMLInstanceTextField.setText(Preferences.getProperty(key));
			} else if (key.equals("xsi_schemeLocation")) {
				mXSDFileTextField.setText(Preferences.getProperty(key));
			} else if (key.equals("workspace_fontsize")) {
				((SpinnerNumberModel) mWorkspaceFontSizeSpinner.getModel()).setValue(Integer.valueOf(Preferences.getProperty(key)));
			}
