        ...
        worker = new PlayerWorker(task) {
            ...
            public void run() {
                ...
                if (syntax.startsWith("[") && syntax.endsWith("]")) {
                    ...
                    // matching something like: agent Action x=2.5 y=0.0 z=-13.0 text='Someone wants a beer!' other=bad some='things' foo='something is negative -0,7' state='Da=fuck.continued and others']
                    final Pattern pattern = Pattern.compile("^\\w+|\\w+\\s|\\w+]|[a-zA-Z-_]+=[a-zA-Z]{1}[a-zA-Z-_]+|\\w+\\=-?[0-9\\.]+|\\w+='[\\w+#-äöüßÄÖÜ\\s:\\.,!?=@/]+'");
                    ...
                    for (SceneUttr uttr : turn.getUttrList()) {
                        ...
                        for (final UttrElement element : uttr.getWordList()) {
                            ...
                            if (element instanceof ActionObject) {
                                ...
                                // 12.8.2020 PG: debug mLogger.message("Registering action " + action.getName() + " from actor " + actor + " with time marker >" + marker + "< for execution by this marker");
                        ...
                        if (isDone()) {
                            mLogger.message("It is interrupted ...");
                            return;
                        }
                    }
                }
