...
install(Name, Time) :-
   retractall(timer(Name, _)), write(retracting), nl,
   now(Now), End is Now + Time,    write(Now), nl,
   assertz(timer(Name, End)),
   write('Timer '), write(Name), write(' Set To '), write(End), nl.
