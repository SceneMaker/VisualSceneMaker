...
//      b.setFocusable(false);
    ...
    private String getPrefixedLineBreakers() {
        ...
        int countLineBreakers = 0;
        for (int i = 0; i < lastTwoCharacters.length(); i++) {
            if (lastTwoCharacters.charAt(i) == '\n') {
                countLineBreakers++;
            }
        }
        countLineBreakers = Math.min(countLineBreakers, MAX_LINE_BREAKERS_ALLOWED);
        int missing = MAX_LINE_BREAKERS_ALLOWED - countLineBreakers;
        if (missing <= 0) {
            return "";
        }
        return "\n".repeat(missing);
...
//            }
    ...
    public void search(String word, String language, JTextComponent comp, Highlighter.HighlightPainter painter) {
        ...
        this.searchOffsets = new ArrayList<>();
