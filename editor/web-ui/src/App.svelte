...
Generate only the scene text. Do not include explanations, markdown formatting, or code blocks.`;
  ...
  // Opens the static script-syntax help page in its own popup window (not a tab) — called
  // synchronously from the click handler so the browser doesn't treat it as a blocked popup.
  // A fixed window name means repeated clicks reuse the same window instead of spawning more.
  function openScriptSyntaxHelp() {
    window.open(
      "/web-ui/scenescript-help.html",
      "vsmScriptSyntaxHelp",
      "width=880,height=920,resizable=yes,scrollbars=yes"
    );
  }
      ...
      {#if selectedProject && scriptAreaVisible}
        <div class="scenescript" bind:this={scenescriptEl}>
          ...
          <div class="script-toolbar">
            ...
            <!-- Embeddings service status. Lives here rather than in the project menu because the
                 service only backs script-side features (Title Generator, Semantic Analysis,
                 scene-title suggestions) — it is status for this toolbar's own tools. Note the
                 badge is a plain flex item here, so it must NOT carry embeddings-badge-centered,
                 which absolutely-centred it in the menu bar it came from. -->
            ...
            {:else if embeddingsAvailable}
              <span
                class="autosave-status embeddings-badge"
                aria-live="polite"
            ...
            <button
              ...
              on:click={openScriptSyntaxHelp}
              title="Open script syntax help"
              aria-label="Open script syntax help"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="16" height="16" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 5.25h.008v.008H12v-.008Z" />
              </svg>
